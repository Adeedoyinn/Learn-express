...
const handleNewUser = async (req, res) => {
    ...
    if (!user || !pword) return res.status(400).json({ "Message": "Username and password was not received" }); //this cheks if the request came in with a username and password

    if (!ROLES_LIST.hasOwnProperty(role)) {
        return res.status(400).json(`{ "Message": Invalid role provided: ${role}. Allowed roles are: ${ROLES_LIST} }`);
    } else {
...
        })
        ...
        console.log(result);
        res.status(201).json({"message": `The person ${user} has been created`});
    }catch (err) {
